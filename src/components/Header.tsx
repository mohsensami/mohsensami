            ...
            <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4">
                <Link href="/" className="group flex items-center gap-2">
                    ...
                    <div>
                        ...
                        <span className="block text-xs text-stone-500">دنیای سادهٔ برنامه‌نویسی</span>
                    </div>
                </Link>

                <nav className="flex items-center gap-3 text-sm">
                    <Link href="/posts" className="hidden text-stone-600 transition hover:text-emerald-700 sm:inline">
                        همه نوشته‌ها
                    </Link>
                    ...
                    {session?.user ? (
                        <>
                            ...
                            <span className="hidden text-stone-500 sm:inline">{session.user.name}</span>
                            <form
                                action={async () => {
                                    'use server';
